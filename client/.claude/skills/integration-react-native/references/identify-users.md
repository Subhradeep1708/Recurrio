...
2.  Generate a short-lived, cryptographically signed, one-time handoff token on your backend that wraps the web distinct ID. Add this handoff token (instead of raw `ph_distinct_id` query parameters) to the deep link URL.
3.  When the user is redirected to the app, parse the deep link and handle it securely:
    -   **Verify the handoff token**: Check the token's cryptographic signature, expiration time, and single-use (replay prevention) status.
    -   **Reject invalid/replayed tokens**: If the token is invalid, expired, or has already been used, reject it immediately and abort without attributing or aliasing any identities.
...
class DeepLinkIdentityManager {
    ...
    func handleDeepLink(_ url: URL, isAuthenticatedOnMobile: Bool) {
        ...
        // Securely verify token's signature, expiration, and single-use status, then extract the web distinct ID
        guard let webDistinctId = TokenVerifier.verifyAndExtractDistinctId(handoffToken) else {
            // Reject invalid or replayed tokens without attributing/aliasing identities
            return
        }
...
object DeepLinkIdentityManager {
    ...
    fun handleDeepLink(uri: Uri, isAuthenticatedOnMobile: Boolean) {
        ...
        // Securely verify token's signature, expiration, and single-use status, then extract the web distinct ID
        val webDistinctId = TokenVerifier.verifyAndExtractDistinctId(handoffToken) ?: return // Reject invalid or replayed tokens
